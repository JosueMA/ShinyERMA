server<-shinyServer(function(input, output){
  ...
  sumtable=data.frame("Statistics" = c("Measure", "N" , "Mean", "SD", "Infit", "Mean", "SD", "Outfit","Mean", "SD", "Reliability of separation"),
    "Items"= c("", NGitem, m_i, sd_i, "", m_infit_i, sd_infit_i, "", m_outfit_i, sd_outfit_i, Rel_i),
...
# Theta Estimates
  ...
  if (is.null(index)==FALSE){
    ...
    names(extremes)<-c("Person ID","percent correct","Measure","Std.err","Infit","Outfit","Fit Category")
  }
  
  if (is.null(index)==FALSE) {PID<-PID[-index]}
  PersonTable <- data.frame(PID, percent, Persons, P.se, Person.infit, Person.outfit, MSE.G)
  names(PersonTable)<-c("Person ID","percent correct","Measure","Std.err","Infit","Outfit","Fit Category")
