mat<-read.csv("https://raw.githubusercontent.com/jwang1223/ShinyERMA/master/HOME.csv",header=TRUE)

server<-shinyServer(function(input, output){
  
  output$Dat<-renderTable({
    inFile<-input$Data
    if (is.null(inFile)) return(mat)
    read.table(inFile$datapath,header=TRUE,sep=input$sep)
  })
  
  output$ItemTable<-renderTable({
    inFile<-input$Data
    if (is.null(inFile)==FALSE){mat<-read.table(inFile$datapath,header=TRUE,sep=input$sep)}
    # Drop item columns
    IMean <- colMeans(mat)
